...
(function (window) {
  ...
  var pkg = _namespace('com.greensock.utils')
  ...
  var _capsExp = /([A-Z])/g
  var _getStyle = function (t, p, cs, str) {
    var result
  ...
  var _emojiStart = 0xD800
  var _emojiEnd = 0xDBFF
  var _emojiLowStart = 0xDC00
  var _emojiRegionStart = 0x1F1E6
  var _emojiRegionEnd = 0x1F1FF
  var _emojiModStart = 0x1f3fb
  var _emojiModEnd = 0x1f3ff
  var _emojiPairCode = function (s) {
    return ((s.charCodeAt(0) - _emojiStart) << 10) + (s.charCodeAt(1) - _emojiLowStart) + 0x10000
  }
  var _isOldIE = (_doc.all && !_doc.addEventListener)
  var _divStart = " style='position:relative;display:inline-block;" + (_isOldIE ? "*display:inline;*zoom:1;'" : "'") // note: we must use both display:inline-block and *display:inline for IE8 and earlier, otherwise it won't flow correctly (and if we only use display:inline, IE won't render most of the property tweens - very odd).
  var _cssClassFunc = function (cssClass, tag) {
    cssClass = cssClass || ''
  ...
  var _pushReversed = function (a, merge) {
    var i = merge.length
    while (--i > -1) {
      a.push(merge[i])
    }
  ...
  var _isBeforeWordDelimiter = function (e, root, wordDelimiter) {
    var next
    while (e && e !== root) {
  ...
  var _setPositionsAfterSplit = function (element, vars, allChars, allWords, allLines, origWidth, origHeight) {
    var cs = _getComputedStyle(element)
    var paddingLeft = _getStyle(element, 'paddingLeft', cs)
    var lineOffsetY = -999
    var borderTopAndBottom = _getStyle(element, 'borderBottomWidth', cs) + _getStyle(element, 'borderTopWidth', cs)
    var borderLeftAndRight = _getStyle(element, 'borderLeftWidth', cs) + _getStyle(element, 'borderRightWidth', cs)
    var padTopAndBottom = _getStyle(element, 'paddingTop', cs) + _getStyle(element, 'paddingBottom', cs)
    var padLeftAndRight = _getStyle(element, 'paddingLeft', cs) + _getStyle(element, 'paddingRight', cs)
    var lineThreshold = _getStyle(element, 'fontSize') * 0.2
    var textAlign = _getStyle(element, 'textAlign', cs, true)
    var charArray = []
    var wordArray = []
    var lineArray = []
    ...
    var lines = (allLines && types.indexOf('lines') !== -1) ? [] : null
    var words = (types.indexOf('words') !== -1)
    var chars = (types.indexOf('chars') !== -1)
    var absolute = (vars.position === 'absolute' || vars.absolute === true)
    var linesClass = vars.linesClass
    var iterateLine = ((linesClass || '').indexOf('++') !== -1)
    var spaceNodesToRemove = []
    var i; var j; var l; var node; var nodes; var isChild; var curLine; var addWordSpaces; var style; var lineNode; var lineWidth; var offset
  ...
  var _splitRawText = function (element, vars, wordStart, charStart) {
    ...
    var wordEnd = vars.span ? '</span>' : '</div>'
    var wordIsOpen = true
    var text; var splitText; var i; var j; var l; var character; var hasTagStart; var emojiPair1; var emojiPair2
