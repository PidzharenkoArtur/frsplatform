...
TheGlobe.CanvasManager = function() {
    var self = this;
    ...
    this.init = function() {
        ...
        this.camera = new GK.Camera(220, this.canvas.width / this.canvas.height, 0.15, 1000.0);


        window.gl = this.canvas.getContext("experimental-webgl", {antialias: false, alpha: false});
        gl.viewport(0, 0, this.canvas.width, this.canvas.height);
        gl.clearColor(0.0, 0, 0.55, 1.0);
    ...
    this.draw = function(timestamp) {
        ...
        sinTime = Math.sin(slowTime/2);

        var m = self.globe.modelMatrix;
        mat4.identity(m);
        mat4.translate(m, m, [8.0, 2.0, -500.0]);
        ...
        mat4.identity(m);
        mat4.translate(m, m, [3.5, -0.0, -90.0]);
        mat4.rotateX(m, m, sinTime);
        mat4.rotateY(m, m, slowTime);
    ...
    this.quakesDidLoad = function(earthquakes) {
        ...
        TheGlobe.canvasManager.quakes.createGeometry(earthquakes);
        TheGlobe.canvasManager.bigQuake.createGeometry(earthquakes[0]);
    };

    this.updateQuakeInfo = function(quake) {
        self.quakeInfoSpan.innerHTML = "<strong>Sold " +
            quake.data.properties.mag + " FRS </strong>" +
    ...
    this.quakePulse = function() {
        ...
        var startPS = TheGlobe.ps * 20.0;
        var endPS = TheGlobe.ps * 10.0;
        ...
        self.infoAnim = new Animation(0.2);
        ...
        self.infoTimeout = setTimeout(function(){
            self.pulse(0.15, 0.04, 0.0, 3.0, dir);
    ...
    this.getPertinentQuake = function() {
        ...
        for (var i=0; i<self.earthquakes.length; i++) {
            var quake = self.earthquakes[i];
    ...
    self.earthquake = function() {
        ...
        a.updateFn = function(value) {
            ...
            self.globe.waveEffectStr = BK.Ease.smoothstep(0.0, 0.1, value) -
